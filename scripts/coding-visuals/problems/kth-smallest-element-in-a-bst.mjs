...
const levels = [['5'], ['3', '7'], ['2', '4', '6', '8'], ['1', '-', '-', '-', '-', '-', '-', '-']];
const state = (index, label, extra) => tree(
  levels,
  [mark(index, label, label.includes('return') ? 'output' : 'focus', 'cursor')],
  { input: 'tree=[5,3,7,2,4,6,8,1], k=4', ...extra },
);

const draft = visual('Iterative inorder exposes BST values in ascending order, so the fourth pop is the answer.', [
  frame('Initialize traversal', 'Set root=5, stack=[], and remaining k=4.', state(0, 'root=5', { stack: '[]', remaining: '4' }), 'initialize'),
  frame('Push 5', 'Push root 5 and move root to its left child 3.', state(1, 'root=3', { stack: '[5]', remaining: '4' }), 'push-five'),
  frame('Push 3', 'Push 3 and move root left to 2.', state(3, 'root=2', { stack: '[5,3]', remaining: '4' }), 'push-three'),
  frame('Push 2', 'Push 2 and move root left to 1.', state(7, 'root=1', { stack: '[5,3,2]', remaining: '4' }), 'push-two'),
  frame('Push 1 and reach null', 'Push 1; its left child is null, so the inner loop stops with 1 on top.', state(7, 'root=null', { stack: '[5,3,2,1] top=1', remaining: '4' }), 'push-one'),
  frame('Pop the first value', 'Pop 1, decrement remaining from 4 to 3, then set root=1.right=null.', state(7, 'visit 1', { stack: '[5,3,2]', arithmetic: '4 - 1 = 3' }), 'visit-one'),
  frame('Pop the second value', 'With root null, pop 2, decrement remaining from 3 to 2, then set root=2.right=null.', state(3, 'visit 2', { stack: '[5,3]', arithmetic: '3 - 1 = 2' }), 'visit-two'),
  frame('Pop the third value', 'Pop 3, decrement remaining from 2 to 1, then move root to 3.right at node 4.', state(4, 'root=4', { stack: '[5]', arithmetic: '2 - 1 = 1', visited: '[1,2,3]' }), 'visit-three'),
  frame('Push the next left spine', 'Push node 4 and move root to 4.left=null.', state(4, 'root=null', { stack: '[5,4] top=4', remaining: '1' }), 'push-four'),
...
const review = {
  ...
  visualRationale: 'The complete tree remains fixed while one cursor moves along real parent-child edges and the visible stack and k arithmetic change at every push and pop.',
