...
const positions = {
  O1: { x: 90, y: 58 }, O2: { x: 42, y: 174 }, O3: { x: 138, y: 174 },
  C1: { x: 390, y: 58 }, C2: { x: 342, y: 174 }, C3: { x: 438, y: 174 },
};
const regions = [{ label: 'original', x: 90, y: 20 }, { label: 'clone', x: 390, y: 20 }];
const state = (nodes, edges, extra) => graph(nodes, edges, { positions, regions, edgeLabels: false, ...extra });
...
const draft = visual('Create one mapped copy per original node, then reproduce every adjacency entry between copies.', [
  frame(
    ...
    state(['O1', 'O2', 'O3', 'C1'], originalEdges, { start: 'O1', current: 'O1', frontier: ['O1'], copies: 'O1->C1' }),
    'seed-copy-map',
  ),
  frame(
    'Process original node O1',
    'Pop O1. Discover O2, create C2, enqueue O2, and append C2 to C1.neighbors.',
    state(['O1', 'O2', 'O3', 'C1', 'C2'], [...originalEdges, 'C1 -> C2'], { start: 'O1', current: 'O1', frontier: ['O2'], copies: 'O1->C1, O2->C2' }),
    'copy-o2',
  ),
  frame(
    'Finish O1 adjacency',
    'O1 next reaches unseen O3: create C3, enqueue O3, and append C3 to C1.neighbors. The queue is [O2,O3].',
    state(['O1', 'O2', 'O3', 'C1', 'C2', 'C3'], [...originalEdges, 'C1 -> C2', 'C1 -> C3'], { start: 'O1', current: 'O1', frontier: ['O2', 'O3'], copies: 'O1->C1, O2->C2, O3->C3' }),
    'copy-o3',
  ),
  frame(
    ...
    state(['O1', 'O2', 'O3', 'C1', 'C2', 'C3'], [...originalEdges, 'C1 -> C2', 'C1 -> C3', 'C2 -> C1', 'C2 -> C3'], { start: 'O2', current: 'O2', frontier: ['O3'], copies: '3 copies; no duplicates' }),
    'connect-c2',
  ),
  frame(
    ...
    state(['O1', 'O2', 'O3', 'C1', 'C2', 'C3'], [...originalEdges, 'C1 -> C2', 'C1 -> C3', 'C2 -> C1', 'C2 -> C3', 'C3 -> C1', 'C3 -> C2'], { start: 'O3', current: 'O3', frontier: [], copies: 'O1->C1, O2->C2, O3->C3' }),
    'connect-c3',
  ),
  frame(
    ...
    state(['O1', 'O2', 'O3', 'C1', 'C2', 'C3'], [...originalEdges, 'C1 -> C2', 'C1 -> C3', 'C2 -> C1', 'C2 -> C3', 'C3 -> C1', 'C3 -> C2'], { current: 'return C1', copies: 'all clone edges stay inside C1,C2,C3', result: 'deep copy rooted at C1' }),
    'return-deep-copy',
  ),
]);
