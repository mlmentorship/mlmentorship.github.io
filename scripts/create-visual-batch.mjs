...
tasks.push({
  ...
  priority: 201,
  status: 'not-started',
  description: `Review all ${entries.length} article outcomes together against docs/VISUAL_LEARNING_SYSTEM.md, their source articles, the deck, and cited primary sources. Do not assume agent licensing claims are correct. Reject decorative visuals, copied figures without verified permission, incorrect equations or geometry, color-only meaning, malformed raw HTML, unreadable labels, duplicate IDs, and unnecessary interaction. Verify that each stated calculation agrees with the geometry actually drawn. Fix shared styles only when multiple articles need the same primitive. Run generated-output validation and normal and disabled-Workbook production builds. Render every new figure in a real browser at desktop light/dark, 390 CSS pixel light/dark, and print media; assert window.innerWidth is exactly 390, because Windows headless Chrome --window-size=390 uses a 500 CSS pixel layout and crops the image. Inspect screenshots for missing glyphs, collisions, clipping, and intentional internal scrolling rather than trusting DOM bounds alone. Confirm the number of implemented plus no-visual outcomes increases by exactly ${expectedCompleteIncrease}, with no unrelated article edits. Run: npm run check && PUBLIC_PREP_TOOLS=false npm run build && npm run build`,
