...
tasks.push({
  ...
  priority: 201,
  status: 'not-started',
  description: `Review all ${entries.length} article outcomes together against docs/VISUAL_LEARNING_SYSTEM.md, their source articles, the deck, and cited primary sources. Do not assume agent licensing claims are correct. Reject decorative visuals, copied figures without verified permission, incorrect equations or geometry, color-only meaning, malformed raw HTML, unreadable labels, duplicate IDs, and unnecessary interaction. Verify that each stated calculation agrees with the geometry actually drawn. Fix shared styles only when multiple articles need the same primitive. Run generated-output validation and normal and disabled-Workbook production builds. Use scripts/check-visual-rendering.mjs with these slugs: ${entries.map((entry) => entry.slug).join(',')}. Render every new figure at desktop light/dark, exact 390 CSS pixel light/dark, and print media; inspect every saved PNG plus scroll-end captures for missing glyphs, collisions, clipping, and hierarchy rather than trusting the automated result alone. Do not use Windows Chrome --window-size=390 as mobile evidence because it crops a 500-pixel layout. Confirm the number of implemented plus no-visual outcomes increases by exactly ${expectedCompleteIncrease}, with no unrelated article edits. Run: npm run check && PUBLIC_PREP_TOOLS=false npm run build && npm run build`,
  ...
  acceptanceCriteria: [
    ...
    'All new visuals pass technical, provenance, pixel-level desktop/mobile/theme/print review at a verified 390 CSS pixel viewport',
